...
{
   T::iterator it = hugeInteger.integer.end() - 1;
...
bool HugeInteger< T >::operator<(const HugeInteger &right) const {
	...
	if (integer.size() > right.integer.size())
		return false;
	T::iterator it1 = integer.end() - 1;
	T::iterator it2 = right.integer.end() - 1;
...
HugeInteger<T> HugeInteger<T>::operator-(const HugeInteger &op2) const {
	...
	T::iterator tmp = difference.integer.end() - 1;
	...
	T::iterator it = difference.integer.end() - 1;
...
HugeInteger<T> HugeInteger<T>::operator*(const  HugeInteger &op2) const {
	...
	T::iterator it = product.integer.end() - 1;
...
HugeInteger<T> HugeInteger<T>::operator/(const HugeInteger<T> &op2) const {
	...
	T::iterator it = quotient.integer.end() - 1;
